...
class PresetsTab {
    ...
    readDom() {
        ...
        self.domDialogCli = $("#presets_cli_dialog");
    ...
    tryLoadPresets() {
        ...
        Promise.all(this.presetsSources.map(p => p.loadIndex().catch(p => {failedToLoad.push(p);})))
            .then(() => {
                ...
                this.presetsSources = this.presetsSources.filter(repo => !failedToLoad.includes(repo));
                return this.checkPresetSourceVersion();
            })
            .then(() => {
                this.presetsSources.forEach(p => this.favoritePresets.addLastPickDate(p.index.presets, p));
    ...
    prepareFilterFields() {
        ...
        this.prepareFilterSelectField(this.selectCategory, getUniqueValues(this.presetsSources, x => x.index.uniqueValues.category), 3);
        this.prepareFilterSelectField(this.selectKeyword, getUniqueValues(this.presetsSources, x => x.index.uniqueValues.keywords), 3);
        this.prepareFilterSelectField(this.selectAuthor, getUniqueValues(this.presetsSources, x => x.index.uniqueValues.author), 1);
        this.prepareFilterSelectField(this.selectFirmwareVersion, getUniqueValues(this.presetsSources, x => x.index.uniqueValues.firmware_version), 2);
        this.prepareFilterSelectField(this.selectStatus, getUniqueValues(this.presetsSources, x => x.index.settings.PresetStatusEnum), 2);
    ...
    isPresetFitSearch(preset, searchParams) {
        if (preset.hidden) {
            return false;
        }
